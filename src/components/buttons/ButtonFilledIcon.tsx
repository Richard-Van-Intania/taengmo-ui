...
export function ButtonFilledIcon({ label, iconify, enable, scale, whenClicked }: Props) {
  return (
    <button
      type="button"
      className={cn("flex w-full items-center justify-center rounded-lg", buttonHeight(scale), enable ? "bg-primary text-on-primary cursor-pointer" : "bg-on-surface-op10 text-on-surface-op38 cursor-not-allowed")}
      disabled={!enable}
      onClick={whenClicked}
    >
